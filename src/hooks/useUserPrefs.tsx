...
export interface UserPreferences {
  ...
  postWideUI?: boolean;
  syncWideUI?: boolean;
  cardStyle?: import("../../types").CardStyle;
  mediaOnly?: boolean;
...
export type PreferenceMap = {
  [K in keyof UserPreferences]: [
    UserPreferences[K],
    import("react").Dispatch<
      import("react").SetStateAction<UserPreferences[K]>
    >,
  ];
};
...
const extractPersistablePrefs = (
  preferenceMap: Partial<PreferenceMap>,
): UserPreferences => {
...
export const useUserPrefs = (
  ...
  // Safe QueryClient usage - use conditional return for SSR
  const [hasQueryClient, setHasQueryClient] = useState(false);

  useEffect(() => {
    // Check if QueryClient is available after hydration
    setHasQueryClient(typeof window !== "undefined");
  }, []);
  ...
  const syncMutation = useMutation({
    ...
    onSuccess: (_, preferences) => {
      ...
      try {
        toast.custom(
          (t) => (
            <ToastCustom t={t} message="Preferences synced" mode="success" />
          ),
          { duration: 2000 },
        );
      } catch (error) {
    ...
    onError: (error: any) => {
      ...
      const errorMessage =
        error.response?.data?.error || "Failed to sync preferences";
      setLastSyncError(errorMessage);

      // Safe toast notification
      try {
        toast.custom(
          (t) => <ToastCustom t={t} message={errorMessage} mode="error" />,
          { duration: 4000 },
        );
      } catch (error) {
        console.warn("Toast notification failed:", error);
    ...
    debounce(
      (preferences: UserPreferences) => {
        if (
          ...
          !prefsEqual(preferences, lastSyncedPrefs.current)
        ) {
          if (isOnlineRef.current && hasQueryClient && syncMutation) {
            ...
            userPrefsStore
              .setItem("unsentPrefs", preferences)
              .then(() => {
  ...
  useEffect(() => {
    ...
    if (
      ...
      userPrefsStore
        .getItem("unsentPrefs")
        .then((queuedPrefs: UserPreferences | null) => {
          if (queuedPrefs) {
            syncMutation.mutate(queuedPrefs);
            userPrefsStore
              .removeItem("unsentPrefs")
              .then(() => {
                setQueuedChanges(0);
  ...
  useEffect(() => {
    ...
    const initializePrefs = async () => {
      try {
        ...
        try {
          const stored = (await userPrefsStore.getItem(
            "preferences",
  ...
  useEffect(() => {
    ...
    if (session?.user && hasQueryClient && syncMutation) {
