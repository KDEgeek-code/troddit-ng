...
import { AppContextValue, RedditPost, PostType } from "../types";
...
export const MainProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const addReadPost = ({ postId, numComments }: { postId: string; numComments: number }) => {
  ...
  const toggleReadPost = async ({ postId, numComments }: { postId: string; numComments: number }) => {
    setReadPosts((read) => {
      if (read?.[postId]) {
  ...
  const [scoreFilterNum, setScoreFilterNum] = useState<number | undefined>();
  const [scoreGreater, setScoreGreater] = useState(true);

  // Create preference map for server sync
  const preferenceMap = {
    // UI settings
    nsfw: [nsfw, setNSFW],
    autoplay: [autoplay, setAutoplay],
    hoverplay: [hoverplay, setHoverPlay],
    wideUI: [wideUI, setWideUI],
    saveWideUI: [saveWideUI, setSaveWideUI],
    postWideUI: [postWideUI, setPostWideUI],
    syncWideUI: [syncWideUI, setSyncWideUI],
    cardStyle: [cardStyle, setCardStyle],
    mediaOnly: [mediaOnly, setMediaOnly],
    columnOverride: [columnOverride, setColumnOverride],
    audioOnHover: [audioOnHover, setaudioOnHover],
    autoHideNav: [autoHideNav, setAutoHideNav],
    uniformHeights: [uniformHeights, setUniformHeights],

    // Media settings
    volume: [volume, setVolume],
    compactLinkPics: [compactLinkPics, setCompactLinkPics],
    preferSideBySide: [preferSideBySide, setPreferSideBySide],
    disableSideBySide: [disableSideBySide, setDisableSideBySide],
    ...
    showUserIcons: [showUserIcons, setShowUserIcons],
    showAwardings: [showAwardings, setShowAwardings],
    showFlairs: [showFlairs, setShowFlairs],
    showUserFlairs: [showUserFlairs, setShowUserFlairs],
    expandedSubPane: [expandedSubPane, setExpandedSubPane],

    // Behavior settings
    infiniteLoading: [infiniteLoading, setInfinitLoading],
    dimRead: [dimRead, setDimRead],
    autoRead: [autoRead, setAutoRead],
    autoSeen: [autoSeen, setAutoSeen],
    disableEmbeds: [disableEmbeds, setDisableEmbeds],
    preferEmbeds: [preferEmbeds, setPreferEmbeds],
    embedsEverywhere: [embedsEverywhere, setEmbedsEveryWhere],
    expandImages: [expandImages, setExpandImages],
    ...
    autoPlayInterval: [autoPlayInterval, setAutoPlayInterval],
    waitForVidInterval: [waitForVidInterval, setWaitForVidInterval],
    autoPlayMode: [autoPlayMode, setAutoPlayMode],
    defaultSortComments: [defaultSortComments, setDefaultSortComments],

    // Filter settings
    seenFilter: [seenFilter, setSeenFilter],
    readFilter: [readFilter, setReadFilter],
    imgFilter: [imgFilter, setImgFilter],
    vidFilter: [vidFilter, setVidFilter],
    galFilter: [galFilter, setGalFilter],
    selfFilter: [selfFilter, setSelfFilter],
    linkFilter: [linkFilter, setLinkFilter],
    imgPortraitFilter: [imgPortraitFilter, setImgPortraitFilter],
    imgLandscapeFilter: [imgLandscapeFilter, setImgLandScapeFilter],
    imgResFilter: [imgResFilter, setImgResFilter],
    imgResXFilter: [imgResXFilter, setImgResXFilter],
    imgResYFilter: [imgResYFilter, setImgResYFilter],
    imgResExactFilter: [imgResExactFilter, setImgResExactFilter],
    scoreFilter: [scoreFilter, setScoreFilter],
    scoreFilterNum: [scoreFilterNum, setScoreFilterNum],
    scoreGreater: [scoreGreater, setScoreGreater]
  };

  // Integrate user preferences sync hook (SSR-compatible)
  const { loading: prefsLoading, lastSyncError } = useUserPrefs(preferenceMap as any);
  ...
  const updateLikes = (i: string, like: boolean | null) => {
    if (posts?.[i]?.data) {
      setPosts((p) => {
        p[i].data.likes = like;
        return p;
  ...
  const updateSaves = (i: string, save: boolean) => {
    if (posts?.[i]?.data) {
      setPosts((p) => {
        p[i].data.saved = save;
        return p;
  ...
  const updateHidden = (i: string, hidden: boolean) => {
    let p = posts;
    if (p?.[i]?.data) {
      setPosts((p) => {
  ...
  useEffect(() => {
    const getSettings = async () => {
      ...
      // Wait for preferences to load before setting ready
      const checkReady = () => {
        if (!prefsLoading) {
          setReady(true);
        } else {
